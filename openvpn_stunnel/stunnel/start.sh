...
CONN_STRING="vpn:1194" # CHANGE THIS
IMAGE_NAME="stunnel"
CONTAINER_NAME="st"
...
generate_ca() {
    ...
    openssl req -new -x509 -days 10000 -batch -rand $CERT_DIR/rnd -config openssl.cnf -out $CA_CERT -keyout $CA_KEY
}
...
generate_signed_certs() {
    ...
    openssl req -new -key $key_path -out $CERT_DIR/req.csr -batch -config openssl.cnf
    openssl x509 -req -in $CERT_DIR/req.csr -CA $CA_CERT -CAkey $CA_KEY -CAcreateserial -out $cert_path -days 10000
}
...
start() {
    ...
    # docker network inspect stunnel >/dev/null 2>&1 || docker network create --driver bridge stunnel
